...
6. 请求内切换是否发生，受固定切换上限、错误状态码、是否指定渠道、候选池规模共同影响。
7. 当前请求内切换上限固定为 `0`，不再通过 `system_settings` 配置。
...
3. 当前固定请求内切换上限需要大于 `0`，但现状固定为 `0`，因此默认不会发生请求内切换。
4. 当前分组下该模型存在其他候选渠道。
